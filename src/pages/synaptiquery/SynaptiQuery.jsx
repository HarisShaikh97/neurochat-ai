...
function SynaptiQuery() {
    ...
    const updateSubscription = async () => {
        ...
        const subscriptionData = {
            ...
            synaptiQueryRemaining: state?.current_package?.synaptiQueryRemaining - 1,
        ...
        <Layout>
            ...
            <div className="h-full flex flex-row">
                {state?.synaptiquery_agreed ? (
                    ...
                    <div className="w-96 flex flex-col gap-10 p-5">
                        <div className="flex flex-col gap-5">
                            <div className="font-bold" style={{fontSize: '22px'}}>Top Use cases for SynaptiQuery</div>
                        ...
                        {state?.synaptiquery_agreed ? (
                            <div className="flex flex-col gap-5 h-full">
                                <div className="flex flex-col gap-10 h-full overflow-y-auto px-10 pb-10">
                                    {selectedChat?.chat?.map((thread, key) => {
                                        return (
                                            <ChatMessage thread={thread} modelType='synaptiquery' refreshChat={refreshChat} selectedChat={selectedChat} setShowTranslationPopup={setShowTranslationPopup} setTranslatedText={setTranslatedText} setShowDeleteMessagePopup={setShowDeleteMessagePopup} setCurrentThread={setCurrentThread} key={key} />
                                        )
                                    })}
                                ...
                                {state?.current_package?.synaptiQueryRemaining > 0 ? (
                                    <form onSubmit={handleSubmit} className="flex flex-row gap-5 items-end px-10">
                                        ...
                                        <div className="flex flex-col items-center gap-2 pb-1">
                                            {state?.current_package?.package === 'BASIC' && <div className="font-black border border-bgblue rounded-full flex items-center justify-center" style={{fontSize: '10px', height: '18px', width: '37px'}}>{state?.current_package?.synaptiQueryRemaining}/5</div>}
                            ...
                            <div className="h-full flex flex-col justify-between px-10">
                                <div className="flex flex-col gap-10">
                                    <div className="font-bold" style={{fontSize: '22px'}}>About SynaptiQuery</div>
                                    <div className="text-gray-500" style={{fontSize: '20px'}}>SynaptiQuery is an interactive chat-based interface for querying neuroscientific literature from PubMed. Users can query using keywords, phrases or questions in natural language. SynaptiQuery provides relevant article abstracts and other information. It simplifies and expedites the literature search process, allowing researchers to focus on their work.</div>
                                </div>
                                <div className="flex justify-end w-full">
                                    <button onClick={() => {dispatch({ type: 'SET_SYNAPTIQUERY_AGREED', payload: true })}} className="bg-bgblue text-white font-bold rounded-3xl" style={{height: '56px', width: '343px', fontSize: '16px'}}>I Understand</button>
                                </div>
                            </div>
                        )}
                    </div>
