...
function PaLM2() {
    ...
    const updateSubscription = async () => {
        ...
        const subscriptionData = {
            ...
            chatGptRemaining: state?.current_package?.chatGptRemaining,
            claudeRemaining: state?.current_package?.claudeRemaining,
            palmRemaining: state?.current_package?.palmRemaining - 1,
    ...
    useEffect(() => {
        if(state?.user_id) {
            axios.get(`${apiBaseUrl}/chat/history/${state?.user_id}?model_type=4`).then((res) => {
                // console.log(res)
    ...
    const refreshAllChats = async () => {
        if(state?.user_id) {
            await axios.get(`${apiBaseUrl}/chat/history/${state?.user_id}?model_type=4`).then((res) => {
                console.log(res)
                if(res?.data?.message || res?.data?.status_code === 404) {
                    // console.log(res?.data)
                    setChats([])
        ...
        <Layout>
            ...
            <div className="h-full flex flex-row">
                {state?.palm_agreed ? (
                    ...
                    <div className="w-96 flex flex-col gap-10 p-5">
                        <div className="flex flex-col gap-5">
                            <div className="font-bold" style={{fontSize: '22px'}}>Top Use cases for PaLM2</div>
                            <div className="rounded-3xl text-gray-500 bg-bgblue bg-opacity-5 px-5 flex flex-col gap-5 py-10" style={{width: '343px', fontSize: '15px'}}>
                                <div className="flex flex-row gap-3">
                                    ...
                                    <div className="flex-1">Write essays and stories</div>
                                </div>
                                <div className="flex flex-row gap-3">
                                    <div className="bg-bgblue h-2 w-2 rounded-full mt-2" />
                                    <div className="flex-1">Create blog posts</div>
                                </div>
                                <div className="flex flex-row gap-3">
                                    <div className="bg-bgblue h-2 w-2 rounded-full mt-2" />
                                    <div className="flex-1">Assistance with Creative writing</div>
                                </div>
                                <div className="flex flex-row gap-3">
                                    <div className="bg-bgblue h-2 w-2 rounded-full mt-2" />
                                    <div className="flex-1">Summarize long paragraphs</div>
                                </div>
                                <div className="flex flex-row gap-3">
                                    <div className="bg-bgblue h-2 w-2 rounded-full mt-2" />
                                    <div className="flex-1">More honest, helpful and harmless conversations in general</div>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div className="flex-1 relative">
                    {showTranslationPopup && (<TranslationPopup setShowTranslationPopup={setShowTranslationPopup} translatedText={translatedText} setTranslatedText={setTranslatedText} modelType="palm2" />)}
                        ...
                        {state?.palm_agreed ? (
                            <div className="flex flex-col gap-5 h-full">
                                <div className="flex flex-col gap-10 h-full overflow-y-auto px-10 pb-10">
                                    {selectedChat?.chat?.map((thread, key) => {
                                        return (
                                            <ChatMessage thread={thread} modelType='palm2' refreshChat={refreshChat} selectedChat={selectedChat} setShowTranslationPopup={setShowTranslationPopup} setTranslatedText={setTranslatedText} setShowDeleteMessagePopup={setShowDeleteMessagePopup} setCurrentThread={setCurrentThread} key={key} />
                                        )
                                    })}
                                    {showWaitingMessage && <WaitingMessage query={lastQuery} modelType="palm2" />}
                                    {showErrorMessage && <ErrorMessage query={lastQuery} modelType="palm2" regenerateResponse={regenerateResponse} />}
                                </div>
                                {state?.current_package?.palmRemaining > 0 ? (
                                    <form onSubmit={handleSubmit} className="flex flex-row gap-5 items-end px-10">
                                        ...
                                        <div className="flex flex-col items-center gap-2 pb-1">
                                            {state?.current_package?.package === 'BASIC' && <div className="font-black border border-bgblue rounded-full flex items-center justify-center" style={{fontSize: '10px', height: '18px', width: '37px'}}>{state?.current_package?.palmRemaining}/5</div>}
                            ...
                            <div className="h-full flex flex-col justify-between px-10">
                                <div className="flex flex-col gap-10">
                                    <div className="font-bold" style={{fontSize: '22px'}}>About PaLM2</div>
                                    <div className="text-gray-500" style={{fontSize: '20px'}}>PaLM2 is an interactive chat-based interface for querying neuroscientific literature from PubMed. Users can query using keywords, phrases or questions in natural language. PaLM2 provides relevant article abstracts and other information. It simplifies and expedites the literature search process, allowing researchers to focus on their work.</div>
                                </div>
                                <div className="flex justify-end w-full">
                                    <button onClick={() => {dispatch({ type: 'SET_PALM_AGREED', payload: true })}} className="bg-bgblue text-white font-bold rounded-3xl" style={{height: '56px', width: '343px', fontSize: '16px'}}>I Understand</button>
                                </div>
                            </div>
                        )}
                    </div>
