...
import FalconWarningPopup from "../../components/falconwarningpopup/FalconWarningPopup"
...
function Falcon() {
    ...
    const updateSubscription = async () => {
        ...
        const subscriptionData = {
            ...
            falconRemaining: state?.current_package?.falconRemaining - 1,
    ...
    useEffect(() => {
        if(state?.user_id) {
            axios.get(`${apiBaseUrl}/chat/history/${state?.user_id}?model_type=3`).then((res) => {
                // console.log(res)
    ...
    const refreshAllChats = async () => {
        if(state?.user_id) {
            await axios.get(`${apiBaseUrl}/chat/history/${state?.user_id}?model_type=3`).then((res) => {
                console.log(res)
                if(res?.data?.message || res?.data?.status_code === 404) {
                    // console.log(res?.data)
                    setChats([])
    ...
    return (
        <Layout>
            ...
            {state?.falcon_agreed && showWarningMessage && <FalconWarningPopup setShowWarningMessage={setShowWarningMessage} />}
            ...
            <div className="h-full flex flex-row">
                ...
                <div className="flex-1 relative">
                    {showTranslationPopup && (<TranslationPopup setShowTranslationPopup={setShowTranslationPopup} translatedText={translatedText} setTranslatedText={setTranslatedText} modelType="falcon" />)}
                        ...
                        {state?.falcon_agreed ? (
                            <div className="flex flex-col gap-5 h-full">
                                <div className="flex flex-col gap-10 h-full overflow-y-auto px-10 pb-10 scrollbar-thin scrollbar-thumb-rounded-xl scrollbar-track-rounded-xl scrollbar-thumb-gray-500 scrollbar-track-gray-200">
                                    ...
                                    {showWaitingMessage && <WaitingMessage query={lastQuery} modelType="falcon" />}
                                    {showErrorMessage && <ErrorMessage query={lastQuery} modelType="falcon" regenerateResponse={regenerateResponse} />}
                                </div>
                                {state?.current_package?.falconRemaining > 0 ? (
                                    <form onSubmit={handleSubmit} className="flex flex-row gap-5 items-end px-10">
                                        ...
                                        <div className="flex flex-col items-center gap-2 pb-1">
                                            {state?.current_package?.package === 'BASIC' && <div className="font-black border border-bgblue rounded-full flex items-center justify-center" style={{fontSize: '10px', height: '18px', width: '37px'}}>{state?.current_package?.falconRemaining}/5</div>}
