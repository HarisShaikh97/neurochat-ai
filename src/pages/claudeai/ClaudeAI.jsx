...
function ClaudeAI() {
    ...
    const updateSubscription = async () => {
        ...
        const subscriptionData = {
            ...
            claudeRemaining: currentSubscription?.claudeRemaining - 1,
    ...
    useEffect(() => {
        if(state?.user_id?.length > 0) {
            axios.get(`${apiBaseUrl}/chat/history/${state?.user_id}?model_type=2`).then((res) => {
                console.log(res)
    ...
    const refreshAllChats = async () => {
        if(state?.user_id?.length > 0) {
            await axios.get(`${apiBaseUrl}/chat/history/${state?.user_id}?model_type=2`).then((res) => {
                console.log(res)
                if(res?.data?.message || res?.data?.status_code === 404) {
                    console.log(res?.data)
                    setChats([])
            ...
            <div className="h-full flex flex-row">
                <div className="w-96 flex flex-col gap-10 p-5">
                    ...
                    {agreed ? (
                        ...
                        <div className="flex flex-col gap-5">
                            <div className="text-lg font-semibold">Top Use cases for Claude.AI</div>
                ...
                <div className="flex-1 relative">
                    {showTranslationPopup && (<TranslationPopup setShowTranslationPopup={setShowTranslationPopup} translatedText={translatedText} setTranslatedText={setTranslatedText} modelType="claudeai" />)}
                            ...
                            <div className="flex flex-col gap-5 h-full">
                                <div className="flex flex-col gap-10 h-full overflow-y-auto px-10 pb-10">
                                    {selectedChat?.chat?.map((thread, key) => {
                                        return (
                                            <ChatMessage thread={thread} modelType='claudeai' refreshAllChats={refreshAllChats} refreshChat={refreshChat} selectedChat={selectedChat} setSelectedChat={setSelectedChat} setShowTranslationPopup={setShowTranslationPopup} setTranslatedText={setTranslatedText} key={key} />
                                        )
                                    })}
                                    {showWaitingMessage && <WaitingMessage query={lastQuery} modelType="claudeai" />}
                                    {showErrorMessage && <ErrorMessage query={lastQuery} modelType="claudeai" regenerateResponse={regenerateResponse} />}
                                </div>
                                {currentSubscription?.claudeRemaining > 0 ? (
